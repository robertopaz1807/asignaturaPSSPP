package com.miteris.tareasasincronas;
...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView TV_mensaje =
                            (TextView) findViewById(R.id.TextView_mensajesAlUsuario);
        Button B_probarHacerDosCosasALaVez =
                            (Button) findViewById(R.id.button_probarComoPodemosHacerOtraCosa);

        B_probarHacerDosCosasALaVez.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.v(TAG_LOG, "...Haciendo otra cosa el usuario sobre el hilo PRINCIPAL " +
                                        "a la vez que carga...");
                Toast toast = Toast.makeText(getApplicationContext(), "...Haciendo otra cosa" +
                        "el usuario sobre el hilo PRINCIPAL a la vez que carga...",
                        Toast.LENGTH_SHORT);
                toast.show();
            }
        });
        ...
        // new DescargarImagenesDeInternetEnOtroHilo(CANCELAR_SI_MAS_DE_100_IMAGENES).execute("Estos " +
        //        "Strings van a variableNoUsada que no usaremos en este ejemplo y podiamos haber " +
        //        "declarado como Void ", " si lo necesitaramos podemos cambiar el String por otro " +
        //        "tipo de datos ", " y podemos añadir más de 4 datos que los de este ejemplo, " +
        //        "todos los que necesitemos ", " y recuerda que se usan como un array, para acceder" +
        //        "en concreto a este usaríamos variableNoUsada[3] ");
        //Arrancamos el AsyncTask. el método "execute" envía datos directamente a doInBackground()
    ...
    private class DescargarImagenesDeInternetEnOtroHilo extends AsyncTask<String, Float, Integer> {
         ...
         * En es ste ejemplo le pasamos un booleano que indica si hay más de 100 archivos
         * o no. Si le pasas true se cancela por la mitad del progreso, si le pasas false
         * seguirá hasta el final sin cancelar la descarga simulada
         */
        ...
        /**
         * Se ejecuta antes de empezar el hilo en segundo plano. Después de este se ejecuta
         * el método "doInBackground" en Segundo Plano
        ...
        /**
         * Se ejecuta después de "onPreExecute". Se puede llamar al hilo Principal con el
         * método "publishProgress" que ejecuta el método "onProgressUpdate" en hilo Principal
         * Se ejecuta en el hilo: EN SEGUNDO PLANO
         * @ param Array con los valores pasados en "execute"
         * @ return devuelve un valor al terminar de ejecutar este segundo plano. Se lo envía y
         * ejecuta "onPostExecute" si ha termiado, o a "onCancelled" si se ha cancelado con "cancel"
         */
        @Override
        protected Integer doInBackground(String... variableNoUsada) {
            ...
            while (!isCancelled() && cantidadImagenesDescargadas < 200) {
                ...
                //isCancelled() comprueba si hemos cancelado con cancel() el hilo en seg. plano.
                cantidadImagenesDescargadas++;
                Log.v(TAG_LOG, "Imagen descargada número " + cantidadImagenesDescargadas +
                ...
                try {
                    ...
                    cancel(true);
                    //Cancelamos si entramos al catch porque algo ha ido mal
                    e.printStackTrace();
                }
                ...
                if (cancelarSiHayMas100Archivos && cantidadImagenesDescargadas > 100) {
                    //Si hemos decidido cancelar al pasar de 100 imágenes descargadas
                    // entramos aquí.
                    cancel(true);
                }
            }
        ...
        protected void onPostExecute(Integer cantidadProcesados) {
            TV_mensaje.setText("DESPUÉS de TERMINAR la descarga. Se han descarcado " +
                    cantidadProcesados + " imágenes. Hilo PRINCIPAL");
            Log.v(TAG_LOG, "DESPUÉS de TERMINAR la descarga. Se han descarcado " +
                    cantidadProcesados + " imágenes. Hilo PRINCIPAL");
        ...
        protected void onCancelled(Integer cantidadProcesados) {
            TV_mensaje.setText("DESPUÉS de CANCELAR la descarga. Se han descarcado " +
                    cantidadProcesados + " imágenes. Hilo PRINCIPAL");
            Log.v(TAG_LOG, "DESPUÉS de CANCELAR la descarga. Se han descarcado " +
                    cantidadProcesados + " imágenes. Hilo PRINCIPAL");
